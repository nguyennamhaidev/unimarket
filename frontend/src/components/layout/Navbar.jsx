...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-slate-200 shadow-sm transition-all">
                    ...
                    <div className="absolute right-0 mt-2 w-60 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                      ...
                      <div className="py-1 text-xs font-medium text-slate-700">
                        ...
                        >
                          ...
                          <span>Thông tin &amp; Đổi mật khẩu</span>
                        </Link>
                        ...
                        {canManageFeatured && (
                          ...
                          >
                            ...
                            <span>{isAdmin ? 'Trang Quản Trị Admin' : 'Quản lý Nổi Bật (QTV)'}</span>
                          </Link>
                        )}
        ...
        {mobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-slate-200 animate-in slide-in-from-top-2 duration-150">
            ...
            <div className="flex flex-col gap-2 text-sm font-semibold text-slate-700">
              <Link to="/products" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-lg hover:bg-slate-100 flex items-center gap-2">
                <span>📦</span>
                <span>Tất cả sản phẩm</span>
              </Link>
              <Link to="/products?category=giao-trinh-sach" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-lg hover:bg-slate-100 flex items-center gap-2">
                <span>📚</span>
                <span>Giáo trình &amp; Sách cũ</span>
              </Link>
              <Link to="/products?category=do-dien-tu-phu-kien" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-lg hover:bg-slate-100 flex items-center gap-2">
                <span>💻</span>
                <span>Đồ điện tử &amp; Laptop</span>
              </Link>
              <Link to="/products?isFree=true" onClick={() => setMobileMenuOpen(false)} className="py-2 px-3 rounded-lg text-emerald-700 bg-emerald-50 font-bold flex items-center gap-2">
                <span>🎁</span>
                <span>Góc Cho Tặng 0đ</span>
              </Link>
      ...
      {/* Quick Tools & Navigation Sub-bar for Desktop */}
      <div className="bg-slate-50/90 border-t border-slate-200/80 hidden md:block">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-9 text-xs font-semibold text-slate-600">
            <div className="flex items-center gap-5 overflow-x-auto no-scrollbar">
              <Link to="/products" className="flex items-center gap-1.5 text-slate-900 font-bold hover:text-emerald-700 transition-colors">
                <Package className="w-3.5 h-3.5 text-emerald-600" />
                <span>Tất cả sản phẩm</span>
              </Link>
              <Link to="/products?category=giao-trinh-sach" className="flex items-center gap-1.5 hover:text-emerald-700 transition-colors">
                <span>📚</span>
                <span>Giáo trình &amp; Sách</span>
              </Link>
              <Link to="/products?category=do-dien-tu-phu-kien" className="flex items-center gap-1.5 hover:text-emerald-700 transition-colors">
                <span>💻</span>
                <span>Đồ điện tử &amp; Laptop</span>
              </Link>
              <Link to="/products?category=do-dung-phong-tro" className="flex items-center gap-1.5 hover:text-emerald-700 transition-colors">
                <span>🪑</span>
                <span>Đồ dùng phòng trọ</span>
              </Link>
              <Link to="/products?isFree=true" className="flex items-center gap-1.5 text-emerald-700 font-bold hover:text-emerald-800 transition-colors">
                <span>🎁</span>
                <span>Góc Cho Tặng 0đ</span>
              </Link>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0 text-[11px] text-slate-500">
              <span className="flex items-center gap-1.5 font-medium text-emerald-700 bg-emerald-100/60 px-2 py-0.5 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-600 animate-pulse"></span>
                <span>Sàn giao dịch sinh viên trực tuyến</span>
              </span>
            </div>
          </div>
        </div>
